...
- Output: DevelopmentCard returned, buyer resources decremented (−1 ORE, −1 WOOL, −1 GRAIN), card added to buyer's hand,
  deck countRemaining() decremented by 1
...
### Method under test:
`playKnightCard(Player player, DevelopmentCard card, int currentRound, Robber robber, int targetHexId, Player victim)`
...
- State: robber's current hex location, victim's resource count, victim adjacency, player's played knight count,
  player's hasPlayedDevCardThisTurn flag
...
### Method under test:
`playMonopolyCard(Player player, DevelopmentCard card, int currentRound, Resource resource, List<Player> otherPlayers)`
...
### Method under test:
`playRoadBuildingCard(Player player, DevelopmentCard card, int currentRound, Edge edge1, Edge edge2)`
...
- State: player's road count (roads already placed), edge1 occupancy, edge1 connectivity, edge2 occupancy, edge2
  connectivity
...
- State road count (Interval [0, 15]): 0 (LOW — 15 remaining); 13 (last pair that fits); 14 (only 1 remaining); 15 (
  HIGH — no roads remaining)
- Output: roads appended to player's road list; card removed from hand
- Output: "Edge cannot be null." / "Edge is already occupied." / "Road must connect to player's existing network." / "No
  roads remaining."
...
### Method under test:
`playYearOfPlentyCard(Player player, DevelopmentCard card, int currentRound, Resource resource1, Resource resource2)`
...
- Input hand (Pointer / Size of Collection): null (CAN'T SET by caller); empty list; 1 VP card; >1 VP card; mixed VP and
  non-VP cards; no VP cards among non-VP cards
